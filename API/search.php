...
$results_per_page = 20;
$final_query = "SELECT * FROM `web_posts` WHERE `verification_status` = '1' ORDER BY `id` LIMIT {$results_per_page }";
...
if (isset($_GET['price_min']) && isset($_GET['price_max'])) {
    ...
    if(!empty($price_min) || !empty($price_max)) {
       ...
       $final_query = "SELECT * FROM `web_posts` WHERE `verification_status` = '1' AND REPLACE(`price`, ',', '') > $price_min AND REPLACE(`price`, ',', '') < $price_max";
    } else {
        $final_query = "SELECT * FROM `web_posts` WHERE `verification_status` = '1' ORDER BY `id` DESC LIMIT {$results_per_page}";    
    }
}
