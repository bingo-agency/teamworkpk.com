...
$thumb_prefix = "thumb_" . time(); //Normal thumb Prefix
...
if (isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if ($image_res) {
        ...
        if (normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality)) {
            ...
            echo '<img src="img/' . $thumb_prefix . $new_file_name . '" alt="Thumbnail">';
//            echo '<br />';
//            echo '<img src="images/' . $new_file_name . '" alt="Resized Image">';
            echo '</div>';

            if (!empty($thumb_prefix) && !empty($new_file_name)) {
                $query = mysqli_query($con,"UPDATE `users` SET `image` = '" . $thumb_prefix . $new_file_name . "' WHERE `id` = '" . $id . "'");
                if ($query) {
                    $db->add_user_activity($con,"Added a new Image");
                    $db->success('Your Image has been updated.');
                }
            }
        }
